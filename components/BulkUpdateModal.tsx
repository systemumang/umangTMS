...
export const BulkUpdateModal: React.FC<BulkUpdateModalProps> = ({ 
  ...
  const handlePreSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'status' && (formData.status === 'In Progress' || formData.status === 'Pending for Client' || formData.status === 'Pending for Owner' || formData.status === 'Pending for Training' || formData.status === 'Pending for Billing' || formData.status === 'Pending for Payment') && (!formData.remarks || formData.remarks.trim() === '')) {
