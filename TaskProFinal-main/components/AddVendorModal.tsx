...
export const AddVendorModal: React.FC<AddVendorModalProps> = ({ isOpen, onClose, onSave, vendors }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Uniqueness check
    if (vendors.some(v => v.name.toLowerCase().trim() === formData.name.toLowerCase().trim())) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-indigo-600">Add Vendor</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-xl flex justify-end space-x-3">
            ...
            <button type="submit" className="px-6 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm">Add Vendor</button>
          </div>
        </form>
      </div>
    </div>
