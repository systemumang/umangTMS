...
interface VendorCategoriesViewProps {
  ...
  onEditCategory: (category: VendorCategory) => void;
}

type SortConfig = {
  key: keyof VendorCategory;
  direction: 'asc' | 'desc';
} | null;
...
export const VendorCategoriesView: React.FC<VendorCategoriesViewProps> = ({ categories, onAddCategory, onDeleteCategory, onEditCategory }) => {
  ...
  const requestSort = (key: keyof VendorCategory) => {
  ...
  const getSortIcon = (key: keyof VendorCategory) => {
  ...
  const handleDeleteClick = (category: VendorCategory) => {
    setSelectedCategory(category);
    ...
    <div className="space-y-6 pb-10">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-black">
              {sortedCategories.map((item, index) => (
               ...
               {sortedCategories.length === 0 && (<tr><td colSpan={3} className="px-6 py-8 text-center text-gray-500">No vendor task categories found.</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className={`space-y-4 md:hidden ${viewMode === 'card' ? 'block' : 'hidden'}`}>
        {sortedCategories.map((item) => (
         ...
         {categories.length === 0 && <div className="text-center py-8 text-gray-500 bg-white rounded-lg border border-gray-200">No vendor task categories found.</div>}
      </div>
      ...
      <ConfirmationModal 
        ...
        message={`Are you sure you want to delete vendor category "${selectedCategory?.name}"?`}
      />
    </div>
  );
};
