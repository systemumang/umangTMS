  ...
  const fetchData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const normalizedRecurringTasks = (data.recurringTasks || []).map((t: any) => ({
            ...t,
            id: Number(t.id),
        ...
        setRecurringTasks(normalizedRecurringTasks);

        const recurringTaskTitleById = new Map<number, string>(
          normalizedRecurringTasks.map((t: any) => [Number(t.id || 0), String(t.title || '')])
        );

        setRecurringActions((data.recurringActions || []).map((a: any) => {
          const taskId = Number(a.taskId || a.taskID || a.taskid || 0);
          const taskTitle = String(a.taskTitle || a.task || a.title || a.TaskTitle || a['Task Title'] || '') || (recurringTaskTitleById.get(taskId) || '');
